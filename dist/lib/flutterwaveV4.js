...
function getClientSecret() {
    return (process.env.FLUTTERWAVE_CLIENT_SECRET ||
...
export function encryptAesGcm(plainText, nonce) {
    const keyB64 = process.env.FLUTTERWAVE_ENCRYPTION_KEY || '';
    if (!keyB64)
        throw new Error('FLUTTERWAVE_ENCRYPTION_KEY is not configured.');
    if (nonce.length !== 12)
        throw new Error('Nonce must be 12 characters.');
...
export function splitName(fullName) {
    ...
    if (parts.length === 0)
        return { first: 'Investor', last: 'User' };
    if (parts.length === 1)
...
/** Sandbox test card — auto-approved via scenario header */
...
export function buildEncryptedCardPayload(card = SANDBOX_TEST_CARD) {
...
export async function initiateDirectCharge(params) {
...
export async function authorizeCharge(chargeId, authorization, scenarioKey) {
    return flwRequest(`/charges/${chargeId}`, {
        method: 'PUT',
        body: { authorization },
        scenarioKey: scenarioKey || (isSandboxMode() ? 'scenario:auth_3ds&issuer:approved' : undefined),
    });
}
...
export async function createDirectTransfer(params) {
    return flwRequest('/direct-transfers', {
        ...
        idempotencyKey: params.reference,
        scenarioKey: isSandboxMode() ? 'scenario:transfer_success&issuer:approved' : undefined,
    });
}
export async function completeSandboxCharge(chargeId) {
