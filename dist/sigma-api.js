...
var LIVE_API = "https://api.flutterwave.com";
...
async function flwRequest(path2, options = {}) {
  ...
  const res = await fetch(`${getApiBase()}${path2}`, {
...
function verifyPassword(password, storedHash) {
  ...
  if (storedHash.length === 64 && /^[a-f0-9]+$/i.test(storedHash)) {
    const legacy = crypto2.createHash("sha256").update(`sigma:${password}`).digest("hex");
...
var SUPABASE_URL = process.env.NEXT_PUBLIC_SUPABASE_URL || process.env.VITE_SUPABASE_URL || "";
var SUPABASE_SERVICE_ROLE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY || "";
var FLUTTERWAVE_ENCRYPTION_KEY = process.env.FLUTTERWAVE_ENCRYPTION_KEY || "";
var FLUTTERWAVE_WEBHOOK_SECRET = process.env.FLUTTERWAVE_WEBHOOK_SECRET || process.env.FLUTTERWAVE_SECRET_HASH || "";
var OPAY_ACCOUNT_NAME = process.env.OPAY_ACCOUNT_NAME || "";
var OPAY_ACCOUNT_NUMBER = process.env.OPAY_ACCOUNT_NUMBER || "";
var OPAY_BANK_NAME = process.env.OPAY_BANK_NAME || "";
var isLiveSupabase = Boolean(
  SUPABASE_URL && SUPABASE_SERVICE_ROLE_KEY && !SUPABASE_URL.includes("your-project") && SUPABASE_SERVICE_ROLE_KEY.length > 40
);
var supabaseAdmin = isLiveSupabase ? createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, {
...
var pendingCharges = /* @__PURE__ */ new Map();
var store = {
  profiles: /* @__PURE__ */ new Map(),
  user_roles: /* @__PURE__ */ new Map(),
  admin_allowlist: /* @__PURE__ */ new Set(),
  bank_details: /* @__PURE__ */ new Map(),
  card_details: /* @__PURE__ */ new Map(),
  investments: /* @__PURE__ */ new Map(),
  payments: [],
  payouts: [],
  opay_receipts: [],
  notifications: [],
...
function ensureReferralCode(profile) {
...
function attachReferral(newProfile, referralCode) {
...
function buildReferralPayload(userId, profile) {
...
function creditReferralCommission(payerProfile, paymentAmount) {
  ...
  if (link) {
    ...
    link.last_earn_at = (/* @__PURE__ */ new Date()).toISOString();
  }
...
function findCanonicalProfile(userId, email) {
...
function linkCanonicalProfile(canonicalId, profileData) {
  ...
  const newProfile = {
    ...
    created_at: profileData.created_at || (/* @__PURE__ */ new Date()).toISOString()
  };
  store.profiles.set(canonicalId, newProfile);
  return newProfile;
}
function getDistinctInvestors() {
  const distinctMap = /* @__PURE__ */ new Map();
  for (const [_, profile] of store.profiles.entries()) {
    const cleanEmail = (profile.email || "").toLowerCase().trim();
    if (!cleanEmail) continue;
    const isAdmin = store.admin_allowlist.has(cleanEmail) || store.user_roles.get(profile.id) === "admin";
    if (isAdmin) continue;
...
async function logActivity(actor, action, details, amount = null) {
  const item = {
...
function computePayoutForInvestment(inv) {
  ...
  return {
    ...
    phase: "week4",
    week: 4,
    label: `Week 4 \xB7 25% + ${(interestRate * 100).toFixed(0)}% interest`,
...
function advanceInvestmentAfterPayout(inv) {
  ...
  if (week < 4) {
    ...
    inv.next_payment_date = addDaysIso(inv.cycle_start_date || /* @__PURE__ */ new Date(), nextWeek * 7);
  } else {
...
function assertMinDeposit(amount) {
  ...
  if (isFlutterwaveConfigured()) {
    try {
      const result = await Promise.race([
        getBanks("NG"),
        new Promise((_, reject) => setTimeout(() => reject(new Error("Banks fetch timed out")), 3e3))
      ]);
      const banks = (result?.data || []).map((b, i) => ({
        id: String(b.id || i + 1),
...
app.get("/api/investor/profile/:id", async (req, res) => {
...
app.post("/api/investor/register-profile", async (req, res) => {
...
app.get("/api/investor/dashboard/:id", async (req, res) => {
  const userId = req.params.id;
  const queryEmail = (req.query.email || "").toLowerCase().trim();
  ...
  if (isLiveSupabase && supabaseAdmin && isValidUUID(userId)) {
    try {
      ...
      const localPayments = store.payments.filter(
        (p) => p.user_id === userId || finalProfile?.id && p.user_id === finalProfile.id || effectiveEmail && p.user_email?.toLowerCase() === effectiveEmail
      );
      const localReceipts = store.opay_receipts.filter(
        (r) => r.user_id === userId || finalProfile?.id && r.user_id === finalProfile.id || effectiveEmail && r.user_email?.toLowerCase() === effectiveEmail
      );
      const localInvestment = store.investments.get(userId) || (finalProfile?.id ? store.investments.get(finalProfile.id) : null);
      const mergedPayments = [...payData || [], ...localPayments.filter((lp) => !(payData || []).some((p) => p.id === lp.id || p.flutterwave_tx_ref && p.flutterwave_tx_ref === lp.flutterwave_tx_ref))];
      const mergedReceipts = [...receiptData || [], ...localReceipts.filter((lr) => !(receiptData || []).some((r) => r.id === lr.id))];
      const approvedReceiptsTotal2 = mergedReceipts.filter((r) => r.status === "approved").reduce((sum, r) => sum + (Number(r.amount) || 0), 0);
      const successfulPaymentsTotal2 = mergedPayments.filter((p) => p.status === "successful").reduce((sum, p) => sum + (Number(p.amount) || 0), 0);
      const totalInvested = Math.max(
        ...
        approvedReceiptsTotal2 + successfulPaymentsTotal2
      );
      ...
      return res.json({
        ...
        notifications: (() => {
          const localNotifs = store.notifications.filter(
            (n) => n.audience === "all" || n.target_user_id === userId || finalProfile?.id && n.target_user_id === finalProfile.id
          );
          const dbNotifs = notifData || [];
          const ids = new Set(dbNotifs.map((n) => n.id));
  ...
  if (!profile) {
    profile = linkCanonicalProfile(userId, {
      id: userId,
      name: (store.current_session?.id === userId ? store.current_session?.name : void 0) || "Investor",
  ...
  const payments = store.payments.filter(
    (p) => p.user_id === userId || p.user_id === profileId || userEmail && p.user_email?.toLowerCase() === userEmail
  );
  const payouts = store.payouts.filter(
    (p) => p.user_id === userId || p.user_id === profileId || userEmail && p.user_email?.toLowerCase() === userEmail
  );
  const opayReceipts = store.opay_receipts.filter(
    (r) => r.user_id === userId || r.user_id === profileId || userEmail && r.user_email?.toLowerCase() === userEmail
  );
  ...
  const approvedReceiptsTotal = opayReceipts.filter((r) => r.status === "approved").reduce((sum, r) => sum + (Number(r.amount) || 0), 0);
  const successfulPaymentsTotal = payments.filter((p) => p.status === "successful").reduce((sum, p) => sum + (Number(p.amount) || 0), 0);
  ...
  if (calculatedTotalInvested > 0) {
    ...
    const phaseToUse = investment?.phase && investment.phase !== "None" ? investment.phase : latestApprovedReceipt?.target_phase || (profile.current_phase !== "None" ? profile.current_phase : "Phase 1: Seed Accumulation");
...
app.post("/api/investor/auto-debit-plan", (req, res) => {
...
app.post("/api/investor/bank-details", (req, res) => {
...
app.post("/api/investor/cancel-subscription", (req, res) => {
...
function isChargeSucceeded(status) {
...
async function creditVerifiedPayment(params) {
  ...
  let profile = userId ? store.profiles.get(userId) : null;
  ...
  try {
    ...
    if (eventType === "transfer.completed") {
      ...
      if (payout) {
        ...
        payout.processed_at = (/* @__PURE__ */ new Date()).toISOString();
        logActivity(
          "Flutterwave Webhook",
      ...
      const customerName = data.customer?.name?.first ? `${data.customer.name.first} ${data.customer.name.last || ""}`.trim() : data.customer?.name || "Investor";
      const pending = txRef ? pendingCharges.get(txRef) : void 0;
...
app.post("/api/flutterwave/sync", async (req, res) => {
  res.json({
    success: true,
    syncedCount: 0,
...
app.post("/api/opay/submit-receipt", async (req, res) => {
...
async function verifyIsAdmin(userId, userEmail) {
  if (isLiveSupabase && supabaseAdmin) {
    try {
      let emailToCheck = (userEmail || "").toLowerCase().trim();
      if (!emailToCheck && isValidUUID(userId)) {
        const { data: profileData } = await supabaseAdmin.from("profiles").select("email").eq("id", userId).maybeSingle();
      ...
      const { data: allowData, error: allowError } = await supabaseAdmin.from("admin_allowlist").select("email").ilike("email", emailToCheck).maybeSingle();
...
app.get("/api/admin/check-role/:id", async (req, res) => {
  const userId = req.params.id;
...
app.get("/api/admin/overview", async (req, res) => {
  ...
  const totalPayoutsDisbursed = store.payouts.filter((p) => p.status === "successful").reduce((sum, p) => sum + Number(p.amount || 0), 0);
  ...
  const statusBuckets = ["successful", "pending", "failed"];
...
app.get("/api/admin/investors", async (req, res) => {
  ...
  let investorsList = distinctInvestors.map((prof) => {
    ...
    const approvedReceiptsTotal = userReceipts.filter((r) => r.status === "approved").reduce((sum, r) => sum + (Number(r.amount) || 0), 0);
    const successfulPaymentsTotal = userPayments.filter((p) => p.status === "successful").reduce((sum, p) => sum + (Number(p.amount) || 0), 0);
    const calculatedTotalInvested = Math.max(
  ...
  if (q && typeof q === "string") {
    ...
    investorsList = investorsList.filter(
      (item) => item.profile.name?.toLowerCase().includes(search) || item.profile.email.toLowerCase().includes(search) || item.profile.phone?.toLowerCase().includes(search)
    );
  }
...
app.delete("/api/admin/investors/:id", async (req, res) => {
  ...
  if (isLiveSupabase && supabaseAdmin && isValidUUID(userId)) {
    try {
      const { data: dbProf } = await supabaseAdmin.from("profiles").select("*").eq("id", userId).maybeSingle();
...
app.post("/api/payouts/run-cron", async (req, res) => {
  ...
  res.json({
    runTimestamp: (/* @__PURE__ */ new Date()).toISOString(),
...
app.post("/api/admin/opay-receipts/review", async (req, res) => {
  ...
  receipt.reviewed_at = (/* @__PURE__ */ new Date()).toISOString();
...
app.get("/api/admin/notifications", async (req, res) => {
  if (isLiveSupabase && supabaseAdmin) {
    try {
      const { data } = await supabaseAdmin.from("notifications").select("*").order("sent_at", { ascending: false }).limit(200);
...
app.post("/api/admin/notifications/send", async (req, res) => {
  ...
  if (isLiveSupabase && supabaseAdmin) {
    try {
      await supabaseAdmin.from("notifications").insert({
        id: isValidUUID(notification.id) ? notification.id : void 0,
        title: notification.title,
        body: notification.body,
        audience: notification.audience,
        target_user_id: notification.target_user_id && isValidUUID(notification.target_user_id) ? notification.target_user_id : null,
